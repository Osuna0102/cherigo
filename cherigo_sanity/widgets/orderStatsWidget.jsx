  ...
  component: () => {
    ...
    useEffect(() => {
      const fetchStats = async () => {
        ...
        const totalCategories = new Set(products.flatMap(product => product.categories)).size

        // Simulate additional data
        const totalSales = Math.floor(Math.random() * 50000) + 10000
        const monthlyProfit = Math.floor(totalSales * 0.35)

        // Simulate top selling items
        const topMonthlyItem = products.length > 0 ?
          { name: products[Math.floor(Math.random() * products.length)].name || 'Product X', sales: Math.floor(Math.random() * 500) + 100 } :
          { name: 'No products', sales: 0 }

        const topAllTimeItem = products.length > 0 ?
          { name: products[Math.floor(Math.random() * products.length)].name || 'Product Y', sales: Math.floor(Math.random() * 2000) + 500 } :
          { name: 'No products', sales: 0 }

        // Simulate shipping data
        const overdueShipments = Math.floor(Math.random() * 10)
        const ordersToShip = Math.floor(Math.random() * 30) + 5
        const orderStatus = {
        ...
        // Calculate sales performance (0-100%)
        const salesPerformance = Math.min(100, Math.floor(Math.random() * 40) + 60)
    ...
    const filteredProducts = stats.products.filter(product =>
      product.categories?.includes(searchCategory)
    )
      ...
      <div style={{ padding: '16px', backgroundColor: '#121212', borderRadius: '8px', boxShadow: '0 1px 3px rgba(0,0,0,0.5)', margin: '0', width: '100%', fontSize: '12px', color: '#e0e0e0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px', marginBottom: '16px' }}>
          ...
          <div style={{ backgroundColor: '#064e3b', borderRadius: '6px', padding: '12px', border: '1px solid #059669', boxShadow: '0 1px 2px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                <p style={{ fontSize: '12px', fontWeight: '500', color: '#6ee7b7' }}>Total Sales</p>
          ...
          <div style={{ backgroundColor: '#78350f', borderRadius: '6px', padding: '12px', border: '1px solid #b45309', boxShadow: '0 1px 2px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                <p style={{ fontSize: '12px', fontWeight: '500', color: '#fcd34d' }}>Monthly Profit</p>
          ...
          <div style={{ backgroundColor: '#7f1d1d', borderRadius: '6px', padding: '12px', border: '1px solid #b91c1c', boxShadow: '0 1px 2px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                <p style={{ fontSize: '12px', fontWeight: '500', color: '#fca5a5' }}>Overdue Shipments</p>
                <h3 style={{ fontSize: '18px', fontWeight: 'bold', color: '#f0f0f0', marginTop: '4px' }}>{stats.overdueShipments}</h3>
              </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', backgroundColor: '#1e1e1e', padding: '12px', borderRadius: '6px', border: '1px solid #333333' }}>
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px', width: '100%' }}>
            ...
            >
              ...
              {Array.from(new Set(stats.products.flatMap(product => product.categories))).map(category => (
              ...
              <div style={{ padding: '12px', backgroundColor: '#1e3a8a', borderRadius: '6px', border: '1px solid #1e40af' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '16px', fontWeight: 'bold', color: '#93c5fd', marginTop: '4px' }}>{stats.orderStatus.shipped}</p>
                  </div>
              ...
              <div style={{ padding: '12px', backgroundColor: '#064e3b', borderRadius: '6px', border: '1px solid #065f46' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '16px', fontWeight: 'bold', color: '#6ee7b7', marginTop: '4px' }}>{stats.orderStatus.delivered}</p>
                  </div>
                  ...
                  {filteredProducts.map((product) => (
                    <tr key={product._id} style={{ borderTop: '1px solid #333333' }}>
                      <td style={{ padding: '8px 16px', whiteSpace: 'nowrap', maxWidth: '250px', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        <div style={{ fontWeight: '500', color: '#e5e7eb' }}>{product.name}</div>
                      </td>
                      <td style={{ padding: '8px 16px', whiteSpace: 'nowrap' }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                          {product.categories?.map(cat => (
                            <span key={cat} style={{ padding: '2px 6px', display: 'inline-flex', fontSize: '10px', fontWeight: '600', borderRadius: '9999px', backgroundColor: '#312e81', color: '#a5b4fc' }}>
                              {cat}
                            </span>
                          ))}
                        </div>
